...
router = APIRouter(prefix="/v1", tags=["v1 — manual"])


@router.post("/query", response_model=QueryResponse)
def query_ask(
    body: QueryRequest,
    usecase: Annotated[QueryAskUsecase, Depends(query_service.query_ask_v1)],
    guardrails: Annotated[Guardrails, Depends(query_service.guardrails)],
) -> QueryResponse:
    """Ask a question about the API integrations documentation.

    This endpoint uses a manual RAG pipeline:
    Voyage AI embeddings → PostgreSQL pgvector retrieval → Claude API.

    Args:
        body: Request with query text and optional conversation history.
        usecase: Injected v1 use case.
    ...
    logger.info("v1 query answered — sources: %s", result.sources)
